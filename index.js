...
const renderFeaturedProducts = ( computers) => {
   let featuredProductsHTML = computers.slice(0,6).map(({id,image, name, oldPrice, newPrice}) =>{
    return `
...
const renderBestSellerproducts = (computers) => {
    let bestSellerproduct = computers.filter((computer) => computer.category === "New Laptops" || computer.category === "Accessory" );

    let bestSellerproductHTML = bestSellerproduct.slice(0,3).map(({id,image, name, oldPrice, newPrice}) =>{
        return `
            <div class="product-card" >
                        <a href="product-details.html?productId=product-card-${id}"><img id="product-card-${id}" src="${image}" alt="${name}"></a>
                        <h3>${name}</h3>
