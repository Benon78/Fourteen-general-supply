...
window.onload = () => {  
    ...
    if (!productId) {  
        ...
        document.getElementById("products").innerHTML = `<img src="images/loading.gif">`;
...
export const getProduct = (productId) => {  
    ...
    const product = computers.find((computer) => computer.id === parseInt(computerId));  
    ...
    const { id, name, oldPrice, newPrice, description, image, subImages } = product;  
...
export const getRelatedProducts = (productId) =>{
    let computerId = productId.slice(-2).replace(/[-]/g, 0);  
    const relatedProduct = computers.find((computer) => computer.id === parseInt(computerId));
    ...
    let relatedComputers = computers.filter((computer) => computer.category === category);
